...
const lines = input.strip.splitLines

proc PartOne =
    var total: int

    for ln in lines:
        var lnseq = ln.findAll(re"\S")
        var res = calc(lnseq)
        echo &"{res} = {ln}"
        total += res.parseInt
...
proc calc(s: seq[string]): string =
    ...
    while i < s.len:
        ...
        if w =~ re"^\d+$":
            case op:
                of "": result = w
                of "+": result = $(result.parseInt + w.parseInt)
                of "*": 
                    if resultMul == "":
                        resultMul = result
                        result = w
                    else:
                        resultMul = $(resultMul.parseInt * result.parseInt)
                        result = w
                else: discard
            op = ""
    ...
    if resultMul != "":
        result = $(resultMul.parseInt * result.parseInt)
