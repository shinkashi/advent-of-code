...
const lns = input.strip.splitLines

type Food = tuple[ing: seq[string], alg: seq[string]]

proc buildFoods: seq[Food] = 
    for ln in lns:
        var i, a: string
        if ln =~ re"^(.+) \(contains (.+)\)$":
            (i, a) = matches
            ...
            raise new Exception
        result.add (ing: i.splitWhitespace, alg: a.split(",").mapIt(it.strip))

proc buildIngredients(foods: var seq[Food]): seq[string] = 
    for f in foods:
        result.add(f.ing)
        result = result.deduplicate

proc buildAllergens(foods: var seq[Food]): seq[string] =
    for f in foods:
        result.add(f.alg)
        result = result.deduplicate
...
# ----------------------------------------------------------------------------------
proc PartOne = 
    var foods = buildFoods()
    var ingredients = buildIngredients(foods).sorted
    var ingredientSet: HashSet[string] = ingredients.toHashSet
    var allergens = buildAllergens(foods).sorted
    var allergenSet: HashSet[string] = allergens.toHashSet

    # var possibleIngs: Table[string, HashSet[string]] # alg -> {ings}

    var safeIngs = ingredientSet

    for alg in allergens:
        var algFoods = foods.filterIt(alg in it.alg)
        var ingSets = algFoods.mapIt(it.ing.toHashSet)
        var ingCommonSet = ingSets.foldl(intersection(a, b))
        echo &"{alg} is in {ingCommonSet}"

        safeIngs.excl(ingCommonSet)

    echo "Safe ings: ", safeIngs
