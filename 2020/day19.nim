...
import strutils, strformat, sequtils, strscans, sugar, math, tables, algorithm, parseutils, re
import memo

const input = inputReal

const blocks = input.strip.split("\n\n")
const ruleBlock = blocks[0].splitLines
const messageBlock = blocks[1].splitLines


proc buildRuleBook(): seq[string] =
    result = newSeq[string](200)
    for rule in ruleBlock:
        if rule =~ re"^(\d+): (.+)$":
            ...
            if desc.startsWith('"'):
                desc = desc[1..1]
            result[num.parseInt] = desc


var ruleBook = buildRuleBook()
...
proc PartTwoSearch(r11rep: int): set[int16] =

    proc getRuleRegex(n: int): string {.memoized.} =
        var R11REGEX = "{" & $r11rep & "}"

        if ruleBook[n] in ["a", "b"]:
            return ruleBook[n]
        
        # Rule 8
        if n == 8:
            return getRuleRegex(42) & "+"

        # Rule 11
        if n == 11:
            return getRuleRegex(42) & R11REGEX & getRuleRegex(31) & R11REGEX

        var parts = ruleBook[n]
                .split("|")
                .mapIt(it.strip.split(" ").map(parseInt))  # like @[@[2, 3], @[3, 2]]

        proc getConcat(s: seq[int]): string =
            "(" & s.map(getRuleRegex).join() & ")"

        result = "(" & parts.map(getConcat).join("|") & ")"


    resetCacheGetRuleRegex()

    var rule0 = re("^" & getRuleRegex(0) & "$")

    var matched: set[int16]
    for i, msg in messageBlock:
        if msg.match(rule0):
            echo &"{i}: {msg} matched"
            matched.incl(int16(i))
    echo &"Matched {matched.len}"
    return matched
...
proc PartTwo =
    ...
    for r in 1..7:
        echo "repetition ", r
        matched = matched + PartTwoSearch(r)

    echo matched
    echo "total matched ", matched.len
